...
# ============================================================
# Elliptic benchmark: variable-coefficient coercive Dirichlet problem
# ============================================================
...
class EllipticProblem(PDEProblem):
    ...
    def plot_prediction(self, pred: np.ndarray, truth: np.ndarray, grid_meta: Dict[str, np.ndarray], outdir: str, tag: str) -> None:
        ...
        for ax, Z, title in zip(axes, [U_true, U_pred, U_err], ["Exact", "Prediction", "Error"]):
...
class BurgersShockProblem(PDEProblem):
    name = "hyperbolic_burgers_shock"
    input_dim = 2
    has_boundary = True
    has_initial = True
    ...
    def extra_metrics(self, pred: np.ndarray, truth: np.ndarray, grid_meta: Dict[str, np.ndarray]) -> Dict[str, float]:
        ...
        # Shock location from threshold halfway between left and right states
        thresh = 0.5 * (self.uL + self.uR)
        pred_idx = int(np.argmin(np.abs(final_pred - thresh)))
        true_idx = int(np.argmin(np.abs(final_true - thresh)))
        shock_loc_pred = xs[pred_idx]
        shock_loc_true = self.s * self.T
        transition_width = float(np.sum((final_pred > self.uR + 0.1 * (self.uL - self.uR)) & (final_pred < self.uL - 0.1 * (self.uL - self.uR))) * (xs[1] - xs[0]))
        osc_measure = float(np.mean(np.maximum(final_pred - self.uL, 0.0) + np.maximum(self.uR - final_pred, 0.0)))
...
class PINNTrainer:
    ...
    def train(self) -> Dict[str, float]:
        cfg = self.config
        t0 = time.time()
        pbar = trange(1, cfg.epochs + 1, desc=f"Training {self.problem.name}", dynamic_ncols=True)
    ...
    def _plot_history(self) -> None:
        ...
        ax.set_title(f"Convergence metrics: {self.problem.name}")
        ax.legend()
        fig.tight_layout()
...
def make_problem(name: str) -> PDEProblem:
    ...
    if name in {"hyperbolic", "burgers", "hyperbolic_burgers_shock"}:
        return BurgersShockProblem(x_left=-1.0, x_right=1.0, T=0.75, uL=2.0, uR=0.0)
...
def default_config(problem_name: str) -> TrainConfig:
    ...
    if problem_name == "elliptic":
        ...
        cfg.epochs = 5000
        cfg.eval_every = 100
    elif problem_name in {"hyperbolic", "hyperbolic_burgers_shock", "burgers"}:
        cfg.n_res = 4096
        cfg.n_bc = 1024
        cfg.n_ic = 2048
        cfg.epochs = 6000
...
def run_single(problem_name: str = "elliptic", seed: int = 0, n_data: int = 0, outdir: str = "baseline_runs") -> Dict[str, float]:
    ...
    cfg = default_config(problem_name if problem_name != "hyperbolic" else "hyperbolic_burgers_shock")
...
if __name__ == "__main__":
    ...
    PROBLEM = "hyperbolic"          # "elliptic", "parabolic", or "hyperbolic"
    SEED = 0
    N_DATA = 100                    # 0 = pure physics-informed baseline
